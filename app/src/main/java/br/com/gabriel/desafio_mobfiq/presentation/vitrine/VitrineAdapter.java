...
public class VitrineAdapter extends RecyclerView.Adapter<VitrineViewHolder> {
    ...
    public void onBindViewHolder(VitrineViewHolder holder, int position) {
        ...
        holder.tv_desconto.setText(String.format("%s\n off",product.getSkus().get(1).getSellers().get(1).getDesconto()));
        holder.tv_name_product.setText(String.format("%s",product.getName()));
        holder.tv_preco_tabela.setText(String.format("%s",product.getSkus().get(1).getSellers().get(1).getListPrice()));
        holder.tv_preco.setText(String.format("%s",product.getSkus().get(1).getSellers().get(1).getPrice()));
        holder.tv_opcao_parcel.setText(String.format("%s de R$ %s",product.getSkus().get(1).getSellers().get(1).getBestInstallment().getCount(),product.getSkus().get(1).getSellers().get(1).getBestInstallment().getValue()));

        holder.progressBar.setVisibility(View.VISIBLE);
        PicassoCache.getPicassoInstance(context).load(product.getImages().get(1).getImageUrl()).into(holder.img_, new Callback() {
            @Override
